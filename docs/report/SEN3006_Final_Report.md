...
- **Applicants' Names:** Abdul Rahman Malak and Mouhammad Houjeirat
...
The rendered diagrams follow. The first three (class, sequence, use case) are mandatory; the remaining four (activity, state, component, deployment) are included because the system has a non-trivial task lifecycle and a modular layered structure.

**Figure 1 — Class Diagram.** Static structure: the `Task`/`AbstractTask` product hierarchy, the `TaskFactory` creator hierarchy (Factory Method), the `PriorityStrategy` algorithm family (Strategy), and the `TaskManager` coordinator that composes both.

![Class Diagram](../uml/images/class-diagram.png)

**Figure 2 — Sequence Diagram.** Runtime interaction for `TaskManager.createTask("BUG", ...)`: factory lookup, delegated instantiation, and registration of the new task.

![Sequence Diagram](../uml/images/sequence-diagram.png)

**Figure 3 — Use Case Diagram.** Developer and Team Lead interactions with the system (create, transition, prioritize, filter, report).

![Use Case Diagram](../uml/images/usecase-diagram.png)

**Figure 4 — Activity Diagram.** End-to-end workflow of a task from creation through prioritization to completion.

![Activity Diagram](../uml/images/activity-diagram.png)

**Figure 5 — State Diagram.** The `TaskStatus` finite state machine (OPEN, IN_PROGRESS, REVIEW, DONE, BLOCKED) and its permitted transitions.

![State Diagram](../uml/images/state-diagram.png)

**Figure 6 — Component Diagram.** Module-level dependencies between the Core, Products, Factories, Strategies, and Coordinator components.

![Component Diagram](../uml/images/component-diagram.png)

**Figure 7 — Deployment Diagram.** The single-JVM runtime environment in which the application executes.
